...
def setup() -> None:
    ...
    resource = Resource.create({"service.name": "text-analysis-backend"})
    _meter_provider = MeterProvider(resource=resource, metric_readers=[reader])
    metrics.set_meter_provider(_meter_provider)

    _meter = _meter_provider.get_meter("text-analysis.model", version="0.1.0")

    _request_counter = _meter.create_counter(
        name="text_analysis_model_requests_total",
        unit="{request}",
        description="Total number of sentiment prediction calls",
    )
    _error_counter = _meter.create_counter(
        name="text_analysis_model_errors_total",
        unit="{error}",
        description="Total number of failed sentiment prediction calls",
    )
    _latency_histogram = _meter.create_histogram(
        name="text_analysis_model_latency_seconds",
        unit="s",
    ...
    _confidence_histogram = _meter.create_histogram(
        name="text_analysis_model_confidence_score",
        unit="{score}",
